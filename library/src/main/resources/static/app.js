...
document.getElementById("register-submit").addEventListener("click", async function(){
    ...
    let newMember = { name: name, email: email, phoneNumber: phone, membershipNumber: membershipNumber };
...
async function loadData() {
    const [booksRes, membersRes, borrowsRes, copiesRes] = await Promise.all([
        fetch('http://localhost:8080/api/books'),
        fetch('http://localhost:8080/api/members'),
        fetch('http://localhost:8080/api/borrows'),
        fetch('http://localhost:8080/api/bookcopies')
    ]);
    books = await booksRes.json();
    members = await membersRes.json();
    borrows = await borrowsRes.json();
    bookCopies = await copiesRes.json();
}
...
document.getElementById("add-book-submit").addEventListener("click", async function(){
    ...
    let newBook = { title, author, isbn, description, publisher };
